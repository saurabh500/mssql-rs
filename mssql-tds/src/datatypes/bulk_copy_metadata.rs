    ...
    pub fn encode(&self, s: &str) -> Vec<u8> {
        match self {
            ...
            EncodingType::CodePage(cp) => {
                ...
                eprintln!("Warning: Code page {cp} not fully supported, using UTF-8");
                s.as_bytes().to_vec()
            }
        }
    }
...
impl From<&crate::query::metadata::ColumnMetadata> for BulkCopyColumnMetadata {
    fn from(col: &crate::query::metadata::ColumnMetadata) -> Self {
        ...
        let (length, type_length, precision, scale) = match &col.type_info.type_info_variant {
            ...
            TypeInfoVariant::VarLen(_var_type, len) => (*len as i32, TypeLength::Variable(*len as i32), 0, 0),
            TypeInfoVariant::VarLenString(len, max_len, _collation) => {
                eprintln!("DEBUG: VarLenString - max_len={}, col_name={}", max_len, col.column_name);
                if *max_len == 0xFFFF {
            ...
            TypeInfoVariant::VarLenScale(len, scale) => (*len as i32, TypeLength::Variable(*len as i32), 0, *scale),
            TypeInfoVariant::VarLenPrecisionScale(len, _max_len, precision, scale) => {
                (*len as i32, TypeLength::Variable(*len as i32), *precision, *scale)
            },
            TypeInfoVariant::PartialLen(_plp_type, unknown_len, _collation, _chunk_size, _plp_null) => {
                // PLP types
