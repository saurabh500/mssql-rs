...
//! TDS framing layer used during the TLS handshake.
//!
//! For TDS 7.4, TLS records exchanged during the prelogin handshake have to be
//! wrapped in TDS prelogin packets. [`TlsOverTdsStream`] sits between the TLS
//! library and the raw transport and performs that framing while
//! `has_completed_tls_handshake` is false. Once the handshake is done it
//! transparently passes bytes through.
//!
//! On macOS, [`BufferedTdsStream`] additionally buffers writes during the
//! handshake so that Security.framework's multiple small writes (one per TLS
//! record in the client flight) coalesce into a single TDS packet.
...
use crate::connection::transport::network_transport::{PRE_NEGOTIATED_PACKET_SIZE, Stream};
use crate::io::packet_writer::PacketWriter;
use crate::message::messages::PacketType;

pub(crate) struct ActiveWriteState {
...
impl ActiveWriteState {
    ...
    pub(crate) const MAX_PACKET_SIZE_WITHOUT_HEADER: usize =
    ...
    fn poll_write_vectored(
        ...
        if self.has_completed_tls_handshake {
            ...
            let external_res = loop {
                ...
                // For named pipes in Message mode, vectored I/O may write header and body
                // separately, causing the 8-byte TDS header to be treated as a complete
                // message and making SQL Server close the pipe. Flatten multiple slices
                // into a single buffer to ensure atomic writes during the handshake.
