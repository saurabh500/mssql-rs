...
use transport_lib::{Config, Parser, Result};
...
fn main() {
    ...
    let host = std::env::args()
        .nth(3)
        .unwrap_or_else(|| std::env::var("CONNECT_HOST").unwrap_or(String::from("localhost:1433")));
    let user = std::env::args()
        .nth(2)
        .unwrap_or_else(|| std::env::var("CONNECT_USER").unwrap_or(String::from("sa")));
